...
if nargin<2 error('Too few input arguments');
    ...
    maxits=1000; convits=100; lam=0.9; plt=0; details=0; nonoise=0; %convits表示聚类结果生成后，再次迭代的话不会改变的次数，也就是在迭代100次的话，结果也是稳定的，则认为收敛了
    i=1;
...
%这是正常的ap算法，所以把稀疏矩阵替换成了相似度矩阵，但是对角线上取的是负无穷
...
while ~dn
	...
	A=A'; R=R';  %原来R和A的一列对应一个聚类中心，转置后，一列对应一个样本
	for ii=1:N,
		old = R(:,ii);
		AS = A(:,ii) + ST(:,ii);
        [Y,I]=max(AS);
	...
	for jj=1:N,
		old = A(:,jj);   %R和A都一列对应一个聚类中心，所以更新availabilities是按列更新的，在每列更新的过程中，由于这一列的对应的k是不变的，所以对应公式Clustering by Passing Messages Between Data Points中的公式2，可以先sum
		Rp = max(R(:,jj),0);
        Rp(jj)=R(jj,jj); %对于公式2的更新，实际上sum（max（0，r（i‘，k）））公式中需要剔除i行和k行的值，然后在加上r（k，k），本行想把整列和r（k，k）sum，然后在剔除i
		A(:,jj) = sum(Rp)-Rp; %剔除i行
        dA = A(jj,jj); %根据公式3，应该剔除k对应的行然后汇总，这实际上是通过sum(Rp)-Rp来实现的，值是相等的e
        A(:,jj) = min(A(:,jj),0); 
        A(jj,jj) = dA;
